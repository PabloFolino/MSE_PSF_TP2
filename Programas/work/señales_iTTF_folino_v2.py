...
# Ejercitación lee información de una archivo "señales_iTTF_folino_v2.pyt"
...
def X_ifft_conFiltro(filtro):
    #--------------------Señal filtrada en frecuencia---------------------------
    X_fft_filtro=np.arange(filtro,dtype=np.complex128)
    F_filtro=np.arange(-filtro/2,filtro/2)
    centro=int(N/2)                                      # Acordrse que N=len(X_fft)
    for i in range(0,filtro):
        X_fft_filtro[i]=X_fft[centro-int(filtro/2)+i]
    M_fft_filtro= abs(X_fft_filtro)/filtro
    #----------Potencia promedio---------
    pot_promedio_filtro=np.sum(M_fft_filtro**2)
    #----------Discreto------------------
    ts_filtro = Ts*np.arange(0,filtro,1)           # Ojo al desconocer Fs --> Ts es ficticio
    #-------Anti Tranformada-------------
...
for i in range(1,4,1):
    delta=1        # Frecuencias que se restan a cada lado el espectro
    filtro=N-2*delta*i
    ts_filtro,señal_filtro,F_filtro,M_fft_filtro=X_ifft_conFiltro(filtro)

    s1 = fig.add_subplot(3,2,(3+i))
    plt.title("Señal en el dominio del tiempo con "+str(filtro)+"muestras")
    plt.xlabel("Im(señal)")
    plt.ylabel("Real(Señal)")
    s1.grid(True)
